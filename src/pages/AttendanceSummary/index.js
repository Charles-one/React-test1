...
const AttendanceSummary = () => {
  ...
  const calculateStatistics = () => {
    ...
    const normalCount = summaryData.filter(
      (item) => item.lateDays === 0 && item.earlyDays === 0 && item.absentDays === 0
    ).length;
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        const rate =
          record.totalDays > 0 ? ((record.normalDays / record.totalDays) * 100).toFixed(2) : '0.00';
        return `${rate}%`;
      },
    },
  ];
    ...
    <div className="attendance-summary">
      <div className="statistics-cards">
        <Row gutter={16}>
          <Col span={6}>
            <Card>
              <Statistic title="总人数" value={stats.total} suffix="人" />
            </Card>
          </Col>
          <Col span={6}>
            <Card>
      ...
      <div className="search-form">
        <Form form={searchForm} layout="inline">
          <Form.Item name="departmentName">
            <Select style={{ width: 160 }} placeholder="选择部门" allowClear>
          ...
          <Form.Item name="employeeId">
            <Select style={{ width: 160 }} placeholder="选择员工" allowClear>
              {employees.map((emp) => (
      ...
      <Table columns={columns} dataSource={summaryData} loading={loading} rowKey="id" />
    </div>
  );
};
